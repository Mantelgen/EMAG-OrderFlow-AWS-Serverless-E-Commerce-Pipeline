...
def lambda_handler(event, context):
    print(f"[SendMail] event: {json.dumps(event)}")
    email = _get_email_from_event_or_db(event) or "<CUSTOMER_EMAIL>"

    order = event["order"]
    ship = event.get("shippingResult", {})
    subject = f"Comanda {order['order_id']} a fost plătită"
    body = (
        f"Bună,\n\nComanda ta {order['order_id']} a fost procesată cu succes.\n"
        f"Cost transport: {ship.get('shipping_cost')} {ship.get('currency','RON')}\n"
        f"Mulțumim!"
    )
